...
interface StageEditTabsProps {
  stage: ModelStage;
  onPatch: (patch: Partial<ModelStage>) => void;
  onEnableTool: (tool: StageTool) => void;
  onDisableTool: (tool: StageTool) => void;
  onReorderTools: (order: StageTool[]) => void;
  onUpdateToolConfig: (config: ToolConfig) => void;
  readOnly?: boolean;
}
...
export default function StageEditTabs({
  stage,
  onPatch,
  onEnableTool,
  onDisableTool,
  ...
  const handleToolConfigChange = (partial: ToolConfig) => {
    onUpdateToolConfig(partial);
  };

  return (
    <div className="h-full flex flex-col">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
        <div className="sticky top-0 bg-white/95 backdrop-blur border-b shadow-[0_1px_0_0_rgba(15,23,42,.06)] z-10">
              ...
              {stage.tools.length === 0 ? (
                ...
                stage.tools.map((tool) => (
                  <ToolConfigForm
                    key={tool}
                    tool={tool}
                    config={stage.toolConfig || {}}
                    onConfigChange={handleToolConfigChange}
                    isDisabled={tool === 'doc_view' && !stage.tools.includes('signatures')}
                  />
                ))
              )}
            </div>
