              ...
              <div className="p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-2xl border shadow-sm bg-white p-4 md:p-6">
                  <header className="flex items-center justify-between mb-4">
                    ...
                    {(() => {
                      const items = [
                        { id: 'data', label: dataPublicacao ? 'Data definida' : 'Definir data de publicação', status: dataPublicacao ? 'completed' as const : 'pending' as const, description: dataPublicacao || 'Selecione a data' },
                        { id: 'meios', label: meiosSelecionados.length > 0 ? 'Meios selecionados' : 'Selecionar meios de publicação', status: meiosSelecionados.length > 0 ? 'completed' as const : 'pending' as const, description: meiosSelecionados.length > 0 ? `${meiosSelecionados.length} meio(s)` : 'Selecione ao menos um' },
                        { id: 'links', label: Object.keys(linksPorMeio).length === meiosSelecionados.length && meiosSelecionados.length > 0 ? 'Links cadastrados' : 'Inserir links/números', status: (meiosSelecionados.length > 0 && Object.keys(linksPorMeio).length === meiosSelecionados.length && Object.values(linksPorMeio).every(v => (v || '').trim())) ? 'completed' as const : (Object.keys(linksPorMeio).length > 0 ? 'warning' as const : 'pending' as const), description: `${Object.values(linksPorMeio).filter(v => (v || '').trim()).length} de ${meiosSelecionados.length} preenchidos` },
                        { id: 'confirm', label: confirmada ? 'Publicação confirmada' : 'Confirmar publicação', status: confirmada ? 'completed' as const : 'pending' as const, description: confirmada ? 'Registro oficial realizado' : 'Confirme para concluir' }
                      ];
