...
// Tipos TypeScript para Assinatura do Edital
...
interface CardAssinaturaEdital {
  processoId: string;
  versaoFinalId: string;  // versão aprovada do Edital
...
interface EditalSignatureSectionProps {
  processoId: string;
  etapaId: number;
...
const mockCardAssinaturaEdital: CardAssinaturaEdital = {
  processoId: "1",
  versaoFinalId: "v1",
  ...
  documentoEdital: {
    nome: "Edital_Versao_Final.pdf",
    url: "mock-url-edital-final",
...
const mockComentarios: Comentario[] = [
  {
    ...
    texto: "Edital aprovado e enviado para assinatura. Aguardando assinaturas dos responsáveis."
  },
  {
    id: "2",
    autor: "Dallas Kelson Francisco de Souza",
    cargo: "Gerente Financeiro",
...
export default function EditalSignatureSection({
  processoId,
  etapaId,
  onComplete,
  onSave,
  ...
  canEdit = true,
  gerenciaCriadora
}: EditalSignatureSectionProps) {
  const { user } = useUser();
  const { toast } = useToast();
  const { podeEditarCard } = usePermissoes();
  
  // Estados principais
  const [cardData, setCardData] = useState<CardAssinaturaEdital>(mockCardAssinaturaEdital);
  ...
  const confirmarConclusao = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "A assinatura do Edital foi concluída com sucesso.",
      });
  ...
  // Função para download do documento Edital
  const handleDownloadEdital = () => {
    if (!cardData.documentoEdital) {
      toast({
        title: "Nenhum documento",
        description: "Nenhum documento de Edital foi enviado ainda.",
        variant: "destructive"
      });
      return;
    }
    ...
    toast({
      ...
      description: `O arquivo ${cardData.documentoEdital.nome} está sendo baixado.`
    });
  };

  // Função para visualizar o documento Edital
  const handleVisualizarEdital = () => {
    if (!cardData.documentoEdital) {
      toast({
        title: "Nenhum documento",
        description: "Nenhum documento de Edital foi enviado ainda.",
    ...
    toast({
      title: "Visualização",
      description: `Abrindo ${cardData.documentoEdital.nome} para visualização.`
    });
  };
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="w-full px-2">
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          {/* ESQUERDA: Visualização do Edital (8 colunas) */}
          <section id="visualizacao-edital" className="col-span-12 lg:col-span-8 w-full">
            <div className="rounded-2xl border shadow-sm overflow-hidden bg-white">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={handleVisualizarEdital}
                      disabled={!cardData.documentoEdital}
                      className="text-xs"
                    >
              ...
              <div className="p-4 md:p-6">
                <div className="space-y-4">
                  ...
                  {cardData.documentoEdital && (
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <FileText className="w-5 h-5 text-blue-600" />
                          <div>
                            <p className="text-sm font-medium text-blue-900">{cardData.documentoEdital.nome}</p>
                            <p className="text-xs text-blue-600">
                              {cardData.documentoEdital.tamanho} • {formatDateBR(cardData.documentoEdital.uploadedAt)} • 
                              Enviado por: {cardData.documentoEdital.uploadedBy}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1">
                          <Button size="sm" variant="outline" onClick={handleVisualizarEdital} className="h-6 w-6 p-0">
                            <Eye className="w-3 h-3" />
                          </Button>
                          <Button size="sm" variant="outline" onClick={handleDownloadEdital} className="h-6 w-6 p-0">
                  ...
                  <div className="w-full min-h-[520px] border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center bg-gray-50">
                    <div className="text-center">
                      ...
                      <p className="text-gray-500 font-medium">Visualização do Edital</p>
                      <p className="text-sm text-gray-400 mt-1">
                        {cardData.documentoEdital ? 'Clique em "Visualizar" para abrir o documento' : 'Nenhum documento disponível'}
                      </p>
                    </div>
                  </div>
          ...
          <section id="comentarios" className="col-span-12 w-full">
            <CommentsSection
              ...
              cardId="comentarios-assinatura-edital"
              title="Comentários"
            />
          </section>
      ...
      <Dialog open={showAssinarModal} onOpenChange={setShowAssinarModal}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Confirme os dados da sua assinatura digital no documento do Edital.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              <div className="text-sm">
                ...
                <div className="text-gray-600">Edital - Versão Final (V{cardData.versaoFinalId})</div>
      ...
      <Dialog open={showAdicionarAssinante} onOpenChange={setShowAdicionarAssinante}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Selecione os usuários que devem assinar o Edital.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Concluir Assinatura do Edital
            </DialogTitle>
            <DialogDescription>
              Confirme a conclusão da etapa de assinatura do Edital.
            </DialogDescription>
          </DialogHeader>
